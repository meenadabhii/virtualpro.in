...
$(function() {
    ...
    var getAns = setInterval(function() {
        if (playing) {
            var maxWidth = window.innerWidth <= 480 ? 335 : 520; // Set maxWidth based on screen width

            if (lose || progressWidth > maxWidth) {
                // console.log('game over');
                //play again
                showResult();
                clearInterval(getAns);
...
//     $('.btn').css({
//         'font-size': Math.min(viewportWidth * 0.1, 100) + 'px'
//     });
//     $('.cal').css({
//         'font-size': Math.min(viewportWidth * 0.05, 40) + 'px'
//     });
//     $('.player').css({
//         'width': Math.min(viewportWidth * 0.9, 425) + 'px'
//     });
// }
